    ...
    useEffect(() => {
        async function run() {
            if(tagsSearch.length < 1) { 
                ...
                const [indicativa, comum] = tagsSearch.reduce(([p, f], e) => (cIndicativaTags.includes(e) ? [[...p, e], f] : [p, [...f, e]]), [[], []]);
                        ...
                        <FlatList
                            ...
                            renderItem={({item}) => (
                                <TouchableOpacity 
                                    ...
                                    style={[styles.searchTagView, cIndicativaTags.includes(item) ? styles.searchTagViewEssential : null]}>
                    ...
                    <Picker
                        ...
                        onValueChange={item => addSearchTag(item)}>
                            {cIndicativaTags.map(item => (
                                <Picker.Item key={item} label={item} value={item} style={styles.filterCIndicativa}/>
                            ))}
            ...
            onPress={() => Alert.alert("Teste")}>
                <View style={explorarItemStyles.container} >
                    <View style={explorarItemStyles.rows}>
                        <Image source={deconvertBase64ToImage(forum.avatar) ?? ''} style={explorarItemStyles.avatar}/>
                        <Text style={explorarItemStyles.title}>{forum.forumName}</Text>
                    </View>
                    <Text style={explorarItemStyles.desc}>{forum.forumDesc}</Text>
                    
                    {explorarItem_tags(forum.tagsDisponiveis, forum.classificacaoIndicativa)}
                </View>
        </TouchableWithoutFeedback>
    )
}
...
function explorarItem_tags(tags, indicativa) {
    ...
    return (
        <View style={[explorarItemStyles.tagView]}>
            <Text style={[explorarItemStyles.tagBody, {backgroundColor: colors.aviso}]}>
                {indicativa}
            </Text>
            {tags.map((tag, index) => (
