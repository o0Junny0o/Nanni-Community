...
const Forum = ({ navigation }) => {
  ...
  const CriarNovoTopico = async () => {
    ...
    const novoForumData = {
      ...
      forumRating: 0, // Valor inicial
      createdAt: new Date(),
    };

    const sucesso = await forumCreate(novoForumData);
    ...
    if (sucesso) {
      ...
      setTopicoDesc('');
      setModal(false);
      carregarTopicosDoForum(); // Recarrega a lista após criar um novo tópico
    } else {
  ...
  const handleDeleteTopico = async (forumID) => {
    ...
    const sucesso = await forumDelete(forumID, user.uid);
    if (sucesso) {
      ...
      {topicos.length === 0 ? (
        ...
        <View style={styles.fullFlex}>
          ...
          <FlatList
            ...
            keyExtractor={(item) => item?.forumID || String(Math.random())}
            renderItem={({ item }) => (
